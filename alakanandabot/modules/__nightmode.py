...
from alakanandabot.mongo import db
        ...
        for c in chats:
            ...
            if present > ctime and valid:
                await tbot.send_message(
                    id,
                    f"""🌒 START NIGHT MODE \n\n❌ From this time up to 7 NO USERS will be able to send text messages in the group.""",
                )
                await tbot(
    ...
    try:
        ...
        for c in chats:
            ...
            if present > otime and valid:
                await tbot.send_message(
                    id,
                    f"""🌒 END NIGHT MODE \n\n ✅ From now on users can again send text messages in the group.""",
                )
                await tbot(
                    functions.messages.EditChatDefaultBannedRightsRequest(
